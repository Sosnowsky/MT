...
Which is the Hamiltonian \ref{Hubbard} without DMI interaction. We can introduce a time dependent electric field in this Hamiltonian by adding a phase to the hopping term $t_{ij}(t) = t_0e^{ie\bs{R}_{ij}\bs{A}(t)}$, where $\bs{A}$ is the vector potential and $\bs{R}_{ij} = \bs{R}_i-\bs{R}_j$. The electric field has the form $\bs{E}(t) = E_0 \sin (\omega t)\bs{\hat{x}}$ (one-dimensional lattice). We can therefore take $\bs{A}(t) = -\frac{E_0}{\omega}\cos(\omega t)\bs{\hat{x}} = -\bs{A}\cos(\omega t)$. Therefore the time dependent Hamiltonian is:
