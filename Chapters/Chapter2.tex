...
Where $\boldsymbol{D}_{ij} = \frac{4it}{\text{U}}\boldsymbol{\Delta}_{ij}$. Lastly, using \ref{SpinRel3}, the term proportional to $\bs{\Delta}_{ij}^2$ gives:
...
\begin{equation}
\hat{H}_{\text{eff}} \approx \sum_{\langle i,j \rangle} \left( J\boldsymbol{S}_i\boldsymbol{S}_j + \boldsymbol{D}_{ij} \boldsymbol{S}_i \times \boldsymbol{S}_j \right) \label{HeffNOEMF}
\end{equation}
