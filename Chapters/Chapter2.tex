...
Introducing these relations into $\hat{H}_J$ and using $n_{i \uparrow} + n_{i \downarrow} = 1$ we obtain:
...
&-\frac{t^2}{\text{U}}\sum_{\langle i,j \rangle, \sigma \sigma'} \left\{ \frac{1}{4}\delta_{\sigma \sigma'} - \sum_{ab} S_i^aS_j^b 	\boldsymbol{\sigma}_{\sigma'\sigma}^a \boldsymbol{\sigma}_{\sigma \sigma'}^b \right\} = 
-\frac{t^2}{\text{U}}\sum_{\langle i,j \rangle} \left\{ \frac{1}{2} - \sum_{ab} S_i^aS_j^b	2\delta_{ab} \right\} = \\
...
Which is the Heisenberg model for antiferromagnets with $J = \frac{2t^2}{\text{U}} > 0$. Likewise, for $\hat{H}_D$:
...
\hat{H}_D = &-\frac{t}{\text{U}} \sum_{\langle i,j \rangle, \sigma_1, \sigma_2, \sigma_3, \sigma_4}\boldsymbol{\Delta}_{ij}(\delta_{\sigma_3 \sigma_4}\boldsymbol{\sigma}_{\sigma_1 \sigma_2} - \delta_{\sigma_1 \sigma_2} \boldsymbol{\sigma}_{\sigma_3\sigma_4})\left(\frac{1}{2}\delta_{\sigma_1 \sigma_4} + \boldsymbol{S}_i\boldsymbol{\sigma}_{\sigma_4 \sigma_1}\right)\left(\frac{1}{2}\delta_{\sigma_2 \sigma_3}-\boldsymbol{S}_j\boldsymbol{\sigma}_{\sigma_2 \sigma_3}\right) = \\
 &= -\frac{t}{\text{U}} \sum_{\langle i,j \rangle} \left\{ \right.
...
\left. \right\} = \\
&= -\frac{2t}{\text{U}}\sum_{\langle i,j \rangle}\left\{ \Delta_{ij}^+ (S_i^-S_j^z - S_i^zS_j^-) + \Delta_{ij}^- (S_i^zS_j^+ - S_i^+S_j^z) + \Delta_{ij}^z (S_i^+S_j^- - S_i^-S_j^+) \right\} = \\
&= +\frac{2ti}{\text{U}}\sum_{\langle i,j \rangle}\left\{ \Delta_{ij}^+ (\bs{S}_i\times\bs{S}_j)^- + \Delta_{ij}^- (\bs{S}_i\times\bs{S}_j)^+ + 2\Delta_{ij}^z (\bs{S}_i\times\bs{S}_j)^z \right\} = \\
