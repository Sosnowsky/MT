...
Where $\bs{D}_{ij}^0 = \frac{4it}{\text{U}}\bs{\Delta}_{ij}$. Lastly, using \ref{SpinRel3}, the term proportional to $\bs{\Delta}_{ij}^2$ gives:
...
With $\bs{\Gamma_{ij}}^{\alpha \beta} = \frac{1}{\text{U}}(\delta_{\alpha \beta} 2\bs{\Delta}_{ij}^2 - 4\Delta_{ij}^\alpha\Delta_{ij}^\beta)$. This term is known as pseudodipolar interaction \citep{Moriya1960} and it will be neglected since it is proportional to $\bs{\Delta}_{ij}^2$.

The total effective Hamiltonian is:

\begin{equation}
\hat{H}_{\text{eff}} \approx \sum_{\langle i,j \rangle} \left( J_{ij}^0 \bs{S}_i\bs{S}_j + \bs{D}_{ij}^0 \bs{S}_i \times \bs{S}_j \right) \label{HeffNOEMF}
\end{equation}
