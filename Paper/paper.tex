...
\documentclass[aps,prl,twocolumn,amsmath,amssymb,superscriptaddress,nobibnotes]{revtex4-1}%
\usepackage{graphicx}
\usepackage{dcolumn}
\usepackage{bm}
\usepackage{color}
...
In Figure \ref{fig2} we show the dependence of $J_{1,ij}'$ and $\bs{D}_{2,ij}'$ in the electric parameter $\mathcal{E} = \frac{eaE_0}{\omega}$ where $e$ is the electron charge and $a$ is the lattice constant. The results are obtained for (units of $\hbar=t_1=1$, typically $t \approx 2.8 \text{eV}$ in graphene \cite{CastroNeto2009}) $t_2 = 0.1$, $\Delta = 0.5$, $\text{U} = 10$ (typical value is $\text{U} \approx 9.3\text{eV}$ in graphene \cite{Wehling2011}) and $\omega = 4, 14$ (equivalently, $T = \frac{2\pi}{\omega} \approx 10^{-16}\text{s}$). 

\begin{figure}[t]
\includegraphics[width=\columnwidth]{../Figures/NNvsNNN.pdf}
\caption{For circularly polarized light we obtain $\tilde{J}_1 = \frac{J_{1,ij}'}{J_{1,ij}}$ and $\tilde{D}_{2} = \frac{D_{2,ij}'}{D_{2,ij}}$ as function of $\mathcal{E} = \frac{eaE_0}{\omega}$. Solid lines are for $\omega = 4$ and dashed lines are for $\omega = 14$.}
\label{fig2}
\vspace*{-6pt}
\end{figure}
