...
Electron dynamics in a 2D planar honeycomb lattice, plotted in Fig. \ref{fig1}, can be described by the Kane-Mele-Hubbard model \cite{Kane2005,PhysRevLett.107.010401,PhysRevLett.106.100403,PhysRevB.82.075106,PhysRevB.84.235149,Griset2012,Auerbach}.
The Hamiltonian in the absence of external perturbations is the sum of the kinetic term $\hat{\mathcal{H}}_t$, the intrinsic SOI $\hat{\mathcal{H}}_{\text{SOI}}$ and the repulsive Coulomb interaction between the electrons modelled by the extended Hubbard interaction $\hat{\mathcal{H}}_{\text{int}}$,
\begin{equation}
\label{MKMH}
...
\end{equation}
where we define the doublon number operator $\hat{d} = \sum_{i=1} \hat{n}_{i\uparrow}\hat{n}_{i\downarrow}$ with eigenvalues $d$. We denote the projection operator to the subspace of states with eigenvalue $d$, i.e., states with exactly $d$ double occupancies, by $\hat{P}_d$ \cite{Fazekas}. We are interested in the strongly correlated regime $U\gg t_{1(2)}$ at half-filling. In such a strong-coupling limit any state with a nonzero number of double occupancies ($d \neq 0$) has a much larger energy than those with no double occupancies ($d=0$). We obtain an effective Hamiltonian acting on the $d=0$ subspace by standard second-order perturbation theory in the hopping terms. Using the following relations,
\begin{align}
