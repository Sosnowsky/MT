...
plot_disconnect(x=field_energies, y = ratioJD, ylim = c(-5*ratioJD0,2*ratioJD0), col = "blue", lwd = 2, xlab = TeX('$E$'), ylab = '')
legend("topright", legend=TeX('$J/D$'), fill = c("blue"))

dev.off()

EMax <- which(field_energies>1)[1]
...
plot(x = res_field_energies, y = theta, lwd = 2, col = "purple", xlab = TeX('$E$'), ylab='')
legend("top", legend=TeX('$\\theta(E)$  '), fill = c("purple"))
